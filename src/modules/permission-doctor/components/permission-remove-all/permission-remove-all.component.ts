...
export class PermissionRemoveAllComponent implements OnInit {
    ...
    getListPermissions() {
        this.permissions$ = this.permissionService.getListPermissionsByRole('doctor').pipe(
            map(response => response.data))
    }

    deletePermission() {
        this.permissionService.deletePermission('doctor',this.nonePermissionIds).subscribe(
            response => {
                Swal.fire({
                    position: 'center',
                    icon: 'success',
